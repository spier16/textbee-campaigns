    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          // Only log errors in production, not verbose debug info
          console.error('Login error:', e?.message || 'Unknown error')

          // Log useful error details without overwhelming the logs
          if (e?.response?.status) {
            console.error('Response status:', e.response.status)
          } else if (e?.code) {
            console.error('Network error code:', e.code)
          }
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          console.error('Register error:', e?.message || 'Unknown error')
          return null
        }
      },
    }),
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          console.error('Google sign-in error:', e?.message || 'Unknown error')
          return null
        }
      },
    }),
