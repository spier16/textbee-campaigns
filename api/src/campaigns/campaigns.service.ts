...
export class CampaignsService {
  ...
  /**
   * Convert a sending window from local timezone to UTC
   *
   * Takes a window with times in local timezone and returns UTC Date objects.
   * This preserves user intent across DST transitions - "9 AM local" stays "9 AM local"
   * year-round, even though the UTC equivalent changes when DST shifts.
   *
   * @param window - SendingWindow with times in local timezone
   * @param timezone - IANA timezone identifier (e.g., "America/Chicago")
   * @returns Object with startDate and endDate as UTC Date objects
   */
  convertLocalWindowToUTC(
    ...
    try {
      ...
      const localEndStr = `${window.endDate}T${window.endTime}:00`

      // Convert from local timezone to UTC Date objects
      // fromZonedTime interprets the string as being IN the specified timezone
      const startDate = fromZonedTime(localStartStr, timezone)
      const endDate = fromZonedTime(localEndStr, timezone)

      this.logger.debug(
        `[TIMEZONE] Converting window: ${window.startDate} ${window.startTime} - ${window.endDate} ${window.endTime} ` +
        `(${timezone}) -> ${startDate.toISOString()} - ${endDate.toISOString()} (UTC)`
      )

      return { startDate, endDate }
    } catch (error) {
      this.logger.error(
        `Error converting window to UTC: ${JSON.stringify(window)}, timezone: ${timezone}`,
        error,
      )
      // Fallback: treat as UTC (will be incorrect but prevents crash)
      return {
        startDate: new Date(`${window.startDate}T${window.startTime}:00Z`),
        endDate: new Date(`${window.endDate}T${window.endTime}:00Z`),
      }
    }
  }
